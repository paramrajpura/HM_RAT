...
events_p={'rippleXhr','spindleXhr','deltaXr'};
animals=[1,2,4,5,7,8];
for focus=1:1
    tab=table;

    tab(1,:)=cell2table({'na','na','na','na','na','na','na','na','na'});
    tab.Properties.VariableNames = ["Animal","StudyDay","Condition","Trial",strcat(events(focus),"_Count"),strcat(events(focus),"_Rate"),"NREM(min)","Thresh Amp","Thresh std"];
    ...
    for i=2:length(a)
        if strfind(a(i).name,'presleep')
            ...
            tab_temp=array2table({animal,studyday,condition,'presleep',size(data.(events{focus}),1),round((size(data.(events{focus}),1))/sleep),round(sleep),round(thr),round(thr_std)});
        ...
        if strfind(a(i).name,'postsleep')
            ...
            tab_temp=array2table({animal,studyday,condition,'postsleep_1',size(data.(events_p{focus}).('hour1'),2),round(size(data.(events_p{focus}).('hour1'),2)/sleep),round(sleep),round(thr),round(thr_std)});
            tab(row,:)=tab_temp;
            ...
            tab_temp=array2table({animal,studyday,condition,'postsleep_2',size(data.(events_p{focus}).('hour2'),2),round(size(data.(events_p{focus}).('hour2'),2)/sleep),round(sleep),round(thr),round(thr_std)});
            tab(row,:)=tab_temp;
            ...
            tab_temp=array2table({animal,studyday,condition,'postsleep_3',size(data.(events_p{focus}).('hour3'),2),round(size(data.(events_p{focus}).('hour3'),2)/sleep),round(sleep),round(thr),round(thr_std)});
            tab(row,:)=tab_temp;
            ...
            tab_temp=array2table({animal,studyday,condition,'postsleep_4',size(data.(events_p{focus}).('hour4'),2),round(size(data.(events_p{focus}).('hour4'),2)/sleep),round(sleep),round(thr),round(thr_std)});
            tab(row,:)=tab_temp;
        end
    end
end
